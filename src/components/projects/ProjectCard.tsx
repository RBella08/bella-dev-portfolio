...
import { fadeInUp, liftHover } from "@/lib/motion";

const MotionLink = motion(Link);

const liftVariants = { rest: { y: 0 }, hover: { y: -6 } };
const zoomVariants = { rest: { scale: 1 }, hover: { scale: 1.06 } };
const fadeVariants = { rest: { opacity: 0 }, hover: { opacity: 1 } };
const shiftVariants = { rest: { x: 0 }, hover: { x: 3 } };
const nudgeVariants = { rest: { x: 0, y: 0 }, hover: { x: 2, y: -2 } };
    ...
    <motion.div variants={fadeInUp} custom={index} className="h-full">
      ...
      >
        <div className="relative aspect-4/3 overflow-hidden">
          <motion.div variants={zoomVariants} transition={liftHover} className="relative h-full w-full">
            <Image
              src={project.image}
        ...
        <div className="flex flex-1 flex-col gap-3 p-6">
          <div className="flex items-start justify-between gap-3">
            <motion.div variants={shiftVariants} transition={liftHover}>
            ...
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-border text-muted-foreground transition-colors duration-300 group-hover:border-accent group-hover:bg-accent group-hover:text-accent-foreground">
              <motion.span variants={nudgeVariants} transition={liftHover} className="flex">
